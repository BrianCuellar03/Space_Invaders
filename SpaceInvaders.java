...
public class SpaceInvaders extends JPanel implements ActionListener, KeyListener, Runnable {
    ...
    private boolean hasWonGame() {
        for(PlayerLaser laser: this.playerLaser){
            for(Moving obj: this.movement) {
                Destroy exp = new Destroy(obj.x, obj.y);
                if(laser.x >= obj.x && laser.x <= obj.x + 15 && laser.y >= obj.y && laser.y <= obj.y + 15){
                    dead.add(exp);
                    movement.remove(obj);
    ...
    private void paintGameScreen(Graphics g) {
        ...
        for (Moving movement: this.movement){
            ...
            if(movement.x==rand.nextInt(550)) {
